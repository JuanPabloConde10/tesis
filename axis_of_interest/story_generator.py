...
from axis_of_interest.schemas import list_of_aoi, PlotSchema
...
from axis_of_interest.prompts import template_prompt_generate_cuento, template_prompt_generate_cuento_gramatica
from axis_of_interest.text_gen import generate_text
from llm_clients.factory import build_client
...
class StoryGenerator:
    ...
    def select_aois(self) -> List[str]:
        ...
        while True:
            try:
                ...
                selected = [aoi_names[i-1] for i in indices if 1 <= i <= len(aoi_names)]
    ...
    def select_llm_provider(self) -> str:
        ...
        while True:
            try:
                choice = input(f"\nSeleccioná proveedor (1-4) [default: {self.settings.default_provider}]: ").strip()
    ...
    def generate_story(
        ...
        span_names = [f"{s.axis_of_interest}/{s.name}" for s in schema_with_names.plots_span]
        ...
        try:
            llm_client = build_client(provider, self.settings)
            
            # Reemplazar el placeholder manualmente para evitar conflictos con {} del JSON
            prompt = template_prompt_generate_cuento_gramatica.replace("{texto_gramatica}", simple_text)
    ...
    def run_interactive(self):
        ...
        if story_gramatica or story_aoi:
            ...
            if save == 's':
                filename = input("Nombre base del archivo (sin extensión): ").strip() or "cuento"
